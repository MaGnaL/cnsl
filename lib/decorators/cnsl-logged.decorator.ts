...
export function cnslLogged({groupTitle, groupCollapsed}: { groupTitle?: string, groupCollapsed?: boolean } = {}): MethodDecorator
{
  return function (target: Object, propertyKey: string, descriptor: TypedPropertyDescriptor<any>) {
    ...
    descriptor.value = function (...args: any[]) {
      ...
      {
        grpCnsl.log(`Call: ${(target.constructor as any).name}.${propertyKey}(${JSON.stringify(args)}) => ${JSON.stringify(result)}`);
      }
